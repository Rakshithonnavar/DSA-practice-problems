...
//     int *a2 = (int *)malloc(m * sizeof(int));  //while using two arrays its important to have dynamic memory allocation.
//     for(int i=0;i<n;i++)
//     {
//      scanf("%d",&a1[i]);
//     }
...
//        scanf("%d",&a2[j]);
//      }
//      int result = intersection(n,a1,m,a2);
  ...
  for(int i=0;i<n;i++)
  {
     if(a[i]==0)
      ...
      int val = (a[i]==1)?1:-1;
           ...
           max_diff = max(max_diff,curMax);
  }
  max_diff = max(0,max_diff);
  return originalzeros+max_diff;
}

int main()
{
  int a[100],n;
  printf("Enter the count of bits");
  scanf("%d",&n);
  printf("Enter the array elements");
  for(int i=0;i<n;i++){
   scanf("%d",&a[i]);
}
  int result = flipBits(a,n);
  printf("The count of ones after flipping are %d",result);
  return 0;
}
